...
為了在網頁應用程式中使用 LINE Login 頻道，必須正確設定 **App types** 與 **Callback URL**。
...
當你成功使用 LINE 憑證登入後，應用程式會顯示你的 LINE 使用者個人資料圖片、顯示名稱與狀態訊息。（如果你使用的是 iOS 或 Android 裝置且已登入 LINE，你將會自動登入。）
